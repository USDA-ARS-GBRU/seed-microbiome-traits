...
Subset the aggregated data to get rid of maternal plant J which does not have any offspring trait data. The result, after removing column 1 (maternal plant identifier) is a 9x1432 matrix for ITS and a 9x650 matrix for 16S.

```{r}
abundance16S_agg_forfitting <- abundance16S_agg_wide[maternal_plant_code %in% pred_grid$maternal_plant_code, -1] |> as.matrix()
abundanceITS_agg_forfitting <- abundanceITS_agg_wide[maternal_plant_code %in% pred_grid$maternal_plant_code, -1] |> as.matrix()
```

We can further remove columns with all 0 counts; there are a few of them in each matrix after maternal plant J was removed. We end up with 9x1293 for 16S and 9x585 for ITS.

```{r}
abundance16S_agg_forfitting <- abundance16S_agg_forfitting[, colSums(abundance16S_agg_forfitting) > 0]
...
melt(rmse_df_16S, variable.name = 'trait', value.name = 'RMSE') |>
  ...
  geom_boxplot() + ggtitle('16S')

melt(rmse_df_ITS, variable.name = 'trait', value.name = 'RMSE') |>
  ggplot(aes(x = trait, y = RMSE)) +
  geom_boxplot() + ggtitle('ITS')
```

These are remarkably high RMSEs, seeing as they are in standard deviation units. A null model should have RMSE = 1 standard deviation. It seems like microbiome does not predict traits. However the results are so poor that I am concerned there is an issue with how the model is being fit. 
...
This code takes only a few minutes to run so I could run it on my laptop.
...
As you can see from the above plots for 16S, most of the prediction results are worse than the null model. In particular the median of the RMSE posterior distribution is well above 1 for all six traits. **Interpretation: When we fit a PLS model to 8/9 of the plants, relating their seedlings' aggregated microbiome to their seedlings' aggregated traits, and predict the held-out maternal plant's average seedling trait value using its average seedling microbiome, we get a worse result than if we had completely ignored microbiome and just made a naive prediction using the mean seedling trait values across all maternal plants.**

It is interesting to see that ITS has a far worse RMSE than 16S. The ITS predictions are extremely bad compared to the null value of RMSE = 1, whereas the 16S predictions are at least clustered slightly above that null value, and some mass of the RMSE posterior distributions are <1 (around 10-20% of iterations had RMSE<1 for 16S, while essentially none did for ITS).

We also see that adding more components from the PLS to the prediction uniformly makes the prediction worse. This is yet another confirmation that there is not that much seedling trait signal in the seedling microbiome data.
