...
> NOTE: So far, I haven't been able to get the R keras package to work on my local Windows machine. I had to run this code on the SciNet cluster which has a Linux OS. Everything works fine there. Regardless of whether I can eventually configure it to run on Windows, if we do end up scaling this up to a larger dataset it is probably going to have to be run on an HPC anyway.
