...
The random forest models performed poorly and do not predict trait given maternal microbiome. 
...
```{r}
abundance_agg_forfitting <- abundance_agg_wide[maternal_plant_code %in% pred_grids[[1]]$maternal_plant_code]
```

Define function to fit the random forest for a single posterior sample. 

```{r}
fit_rf <- function(y, pred_grid) {
...
post_rf_fits <- map2(trait_post_blups_subsample, pred_grids, ~ apply(.x, 1, fit_rf, pred_grid = .y))
```

Examine the out-of-bag R-squared values for all 100 posterior samples.

```{r}
rsqs <- map2(post_rf_fits, trait_names, ~ data.frame(trait = .y, rsquared = map_dbl(.x, 'r.squared'))) |> list_rbind()
ggplot(rsqs, aes(x = rsquared)) +
  geom_histogram() +
