...
Convert abundance data to longform and then create identifier columns for sample ID and maternal plant code. Remove maternal plant J, which does not have any seedling trait data. Sum the counts, then remove all taxa with 0 abundance across all samples. Finally convert to a wideform with taxa as columns. Repeat this for 16S and ITS abundance. 
...
abundance16S_long <- abundance16S_long[maternal_plant_code %in% pred_grids[[1]]$maternal_plant_code]
...
abundance16S_agg_forfitting <- dcast(abundance16S_agg,  maternal_plant_code ~ V1)
...
abundanceITS_long <- abundanceITS_long[maternal_plant_code %in% pred_grids[[1]]$maternal_plant_code]
...
abundanceITS_agg_forfitting <- dcast(abundanceITS_agg,  maternal_plant_code ~ V1)
```
